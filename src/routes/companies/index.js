				...
				else if(req.query.format =="excel")
				{
					...
					try
					{
						...
						for (var i = 0; i < customersData.length; i++) {
							worksheet.addRow({
											  code: customersData[i].code, firm_name: customersData[i].name, first_name: customersData[i].address.first_name,
											  last_name: customersData[i].address.last_name, accounting_name: customersData[i].invoicing_name, 

											  address1: customersData[i].address.address1, address2: customersData[i].address.address2, address3: customersData[i].address.address3,
											  city: customersData[i].address.city, state: customersData[i].address.state, phone: customersData[i].address.phone1,
											  email: customersData[i].address.email1, other_phone: customersData[i].address.phone2, 
											  
											  ship_address1: customersData[i].ship_address.address1, ship_address2: customersData[i].ship_address.address2, ship_address3: customersData[i].ship_address.address3,
											  ship_city: customersData[i].ship_address.city, ship_state: customersData[i].ship_address.state, ship_phone: customersData[i].ship_address.phone1,
											  ship_email: customersData[i].ship_address.email1, ship_other_phone: customersData[i].ship_address.phone2, 
											  
											  bill_address1: customersData[i].bill_address.address1, bill_address2: customersData[i].bill_address.address2, bill_address3: customersData[i].bill_address.address3,
											  bill_city: customersData[i].bill_address.city, bill_state: customersData[i].bill_address.state, bill_phone: customersData[i].bill_address.phone1,
											  bill_email: customersData[i].bill_address.email1, bill_other_phone: customersData[i].bill_address.phone2, 

											  rate_category: customersData[i].custom_type_name, agent: customersData[i].agent.name, agent_code: customersData[i].agent.code,
											  payment_term: customersData[i].payment_term.description, transporter: customersData[i].transporter.name, 
											  gst_type: customersData[i].gst_registration_type, gst_number: customersData[i].gst_number,
											  credit_limit: customersData[i].allowed_balance, current_balance: customersData[i].current_balance, current_overdue: customersData[i].current_overdue,
											  pan_number: customersData[i].pan_number, notes: customersData[i].notes,
											  login_name: customersData[i].user.login_name, password: customersData[i].user.password,
...
function createCustomer (req, res, next) {
	...
	SessionController.validate(sid, function (err, response) {
        ...
        CompanyController.createCustomer(
        	req.body.customer,
        	companyid,
...
function deleteCustomer (req, res, next) {
	...
	SessionController.validate(sid, function (err, response) {
		...
		CompanyController.deleteCustomer(
			req.params.id,
			companyid,
			response.data.session,
			function(err, response) {
...
function updateCustomerBalance (req, res, next) {
	...
	SessionController.validate(sid, function (err, response) {
        ...
        //company.code = Util.randomString(32);
        // save the company and check for errors
        CompanyController.updateCustomerBalance(
        	req.body,
        	companyid,
        	response.data.session,
			function(err, response) {
...
function updateCustomerData (req, res, next) {
	...
	SessionController.validate(sid, function (err, response) {
        ...
        // save the company and check for errors
        CompanyController.updateCustomerData(
        	companyid,
        	req.params.id,
        	req.body,
        	response.data.session,
		...
		CompanyController.findAllAgents (companyid, options, response.data.session, function (err, response) {
			...
			else if(req.query.format =="excel")
			{
				...
				for (var i = 0; i < agentsData.length; i++) {
					...
					worksheet.addRow({code: agentsData[i].code, firm_name: agentsData[i].name, 
									  first_name: agentsData[i].address.first_name, last_name: agentsData[i].address.last_name, 
									  accounting_name: agentsData[i].accounting_name,
									  address1: agentsData[i].address.address1, address2: agentsData[i].address.address2, 
									  address3: agentsData[i].address.address3, city: agentsData[i].address.city, 
									  state: agentsData[i].address.state, email: agentsData[i].address.email1,
									  phone: agentsData[i].address.phone1, other_phone: agentsData[i].address.phone2,
									  commission: agentsData[i].commission, sp_name: agentsData[i].sales_person.first_name + " " + agentsData[i].sales_person.last_name,
									  sp_login_name: agentsData[i].sales_person.login_name, login_name: agentsData[i].user.login_name,
									  password: '', status: agentsData[i].status_id == 4600 ? 'Active' : 'Inactive'
									});
				}
		...
		CompanyController.findAllAgents (companyid, options, response.data.session, function (err, response) {
			...
			else
			{
				...
				if(req.query.format =="excel")
				{
					...
					for (var i = 0; i < agentsData.length; i++) {
						worksheet.addRow({name: agentsData[i].name, city: agentsData[i].address.city, phone: agentsData[i].address.phone1,
												email: agentsData[i].address.email1, sales_person: agentsData[i].sales_person.first_name + " " + agentsData[i].sales_person.last_name});
					}
...
function getAgentValidHeaderList() {
		...
		validHeaderList.push({
			name: "Sales Person", type: "string", length: 32
		});
...
function getCustomerValidHeaderList() {
		...
		validHeaderList.push({
			name: "Customer Type", type: "string", length: 32
		});
		...
		validHeaderList.push({
			name: "AgentID", type: "string", length: 32
		});
...
function listCompanies (req, res, next) {
	...
	SessionController.validate(sid, function (err, response) {
		...
		if(response.data.session.sessiontype != Util.SessionTypesEnum.API && response.data.session.company_id != 1)
			return res.json(Util.setErrorResponse(-100, "Invalid Access"))
