...
def tally_createCustomer(customer):
	...
	try: 
		...
		customer["order_notifications"] = [n for n in customer["notifications"] if n["notification_type_id"] == 5801][0]
		customer["payment_reminders"] = [n for n in customer["notifications"] if n["notification_type_id"] == 5803][0]

		customer["order_notification_mobile"] = "" if customer["order_notifications"]["phone_number"] == None else customer["order_notifications"]["phone_number"]
		customer["order_notification_email"] = "" if customer["order_notifications"]["emails"] == None else customer["order_notifications"]["emails"]

		customer["payment_reminder_mobile"] = "" if customer["payment_reminders"]["phone_number"] == None else customer["payment_reminders"]["phone_number"]
		customer["payment_reminder_email"] = "" if customer["payment_reminders"]["emails"] == None else customer["payment_reminders"]["emails"]
		values = """<ENVELOPE>
			<HEADER>
				<VERSION>1</VERSION>
				...
				<DATA>
					<TALLYMESSAGE>
						<LEDGER Action="Alter" Name=\"""" + escape(customer["name"]) + """\">
							...
							<UDFSIMPLYORDERNOTIFICATIONMOBILE>""" + escape(str(customer["order_notification_mobile"])) + """</UDFSIMPLYORDERNOTIFICATIONMOBILE>
							<UDFSIMPLYORDERNOTIFICATIONEMAIL>""" + escape(str(customer["order_notification_email"])) + """</UDFSIMPLYORDERNOTIFICATIONEMAIL>
							<UDFSIMPLYPAYMENTREMINDERMOBILE>""" + escape(str(customer["payment_reminder_mobile"])) + """</UDFSIMPLYPAYMENTREMINDERMOBILE>
							<UDFSIMPLYPAYMENTREMINDEREMAIL>""" + escape(str(customer["payment_reminder_email"])) + """</UDFSIMPLYPAYMENTREMINDEREMAIL>
							<LANGUAGENAME.LIST>
...
def tally_createUpdateCustomer(customerList, sessionid):
	...
	try:
		...
		for customer in customerList:
			...
			hStatus = tally_isCustomerExists(customer["code"])
...
def tally_isCustomerExists(code):
				...
				<DESC>
					...
					<TDL>
						<TDLMESSAGE>
		                    ...
		                    <SYSTEM TYPE="Formulae" NAME="MYFILTER" ISMOIFY="NO" ISFIXED="NO" ISINTERNAL="NO">$UDFSimplyTextileCustomerCode = \"""" + str(code) + """\"</SYSTEM>
						</TDLMESSAGE>
					</TDL>
				</DESC>
			</BODY>
...
def api_getCustomers (sessionid):
	...
	try:
		...
		response = util.invokeHttpMethod("GET", '/api/customers?enabled_only=1&sync_status_id=4100', "", headers, conn)
		util.closeHttpConnection(conn)
